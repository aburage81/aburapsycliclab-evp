...
import os
import time
from functools import lru_cache# --- 1. Reddit RSS用ヘッダー付きパース ---
def parse_with_headers(url):
    ...
    return feedparser.parse(url, agent=headers['User-Agent'])# --- 2. 翻訳キャッシュとレート制限対策 ---
translator = GoogleTranslator(source='auto', target='ja')@lru_cache(maxsize=500)
def safe_translate(text):
    ...
    try:
        ...
        print(f" 翻訳エラー: {e} -> 原文を使用します")
        return textdef get_sources():
    ...
    return sourcesdef generate_tags(text):
    tags = []
    low_text = text.lower()
    keywords = {
    ...
    for tag, keys in keywords.items():
        if any(k in low_text for k in keys):
            tags.append(tag)
    return " ".join(tags) if tags else "#Paranormal"def crawl():
    jst = timezone(timedelta(hours=9), 'JST')
    now_str = datetime.datetime.now(jst).strftime("%Y-%m-%d %H:%M")# 絶対パスの取得
...
print(f" 更新成功: {len(unique_new)}件の新信号をキャッチ")if __name__ == "__main__":
    crawl()
