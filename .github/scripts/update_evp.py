...
def crawl():
    ...
    html_path = "index.html"   # Actionsではリポジトリルートに置く想定

    new_posts = []
    print(f"🌍 高頻度スキャン開始 - {now_str}")

    for url in get_sources():
        print(f"🔍 取得中: {url}")
        try:
            # Reddit対策ヘッダー
            feed = feedparser.parse(url, request_headers={'User-Agent': 'Mozilla/5.0 (compatible; ParanormalBot)'})
